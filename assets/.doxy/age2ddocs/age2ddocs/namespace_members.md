
# Namespace Members



## a

* **Assert** ([**AGE::CoreLogger**](namespace_a_g_e_1_1_core_logger.md), [**AGE::GameLogger**](namespace_a_g_e_1_1_game_logger.md))
* **Axes** ([**AGE::GamePad**](namespace_a_g_e_1_1_game_pad.md))
* **AGEImageFormatToGLDataFormat** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **AGEImageFormatToGLInternalFormat** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **AGETextureFormatToGL** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **AttachColorTexture** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **AttachDepthTexture** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **AGENodeType** ([**AGE**](namespace_a_g_e.md))
* **AGEPinType** ([**AGE**](namespace_a_g_e.md))
* **Active** ([**AGE**](namespace_a_g_e.md))
* **AseVector** ([**AGE**](namespace_a_g_e.md))
* **AsepriteChunkType** ([**AGE**](namespace_a_g_e.md))
* **AsepritePropertyTypes** ([**AGE**](namespace_a_g_e.md))
* **AudioEngineType** ([**AGE**](namespace_a_g_e.md))
* **AudioFileFormat** ([**AGE**](namespace_a_g_e.md))
* **a** ([**AGE**](namespace_a_g_e.md))
* **another** ([**AGE**](namespace_a_g_e.md))


## b

* **Buttons** ([**AGE::GamePad**](namespace_a_g_e_1_1_game_pad.md), [**AGE::Mouse**](namespace_a_g_e_1_1_mouse.md))
* **BindTexture** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **BeginScene** ([**AGE**](namespace_a_g_e.md))
* **BodyType** ([**AGE**](namespace_a_g_e.md))
* **b2World\_OverlapShape** ([**AGE**](namespace_a_g_e.md))
* **bProgramRunning** ([**AGE**](namespace_a_g_e.md))


## c

* **Critical** ([**AGE::CoreLogger**](namespace_a_g_e_1_1_core_logger.md), [**AGE::GameLogger**](namespace_a_g_e_1_1_game_logger.md))
* **CleanupOutputString** ([**AGE::InstrumentorUtils**](namespace_a_g_e_1_1_instrumentor_utils.md))
* **ConvertAPIToString** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **ConvertToString** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **CreateTextures** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **COMMENT** ([**AGE**](namespace_a_g_e.md))
* **CharMovementStatus** ([**AGE**](namespace_a_g_e.md))
* **ConstantBufferStruct** ([**AGE**](namespace_a_g_e.md))
* **ConstantBufferStruct2D** ([**AGE**](namespace_a_g_e.md))
* **CopyComponent** ([**AGE**](namespace_a_g_e.md))
* **CopyComponentIfExists** ([**AGE**](namespace_a_g_e.md))
* **CreateAndCacheAtlas** ([**AGE**](namespace_a_g_e.md))
* **CreateApp** ([**AGE**](namespace_a_g_e.md))
* **CreateRef** ([**AGE**](namespace_a_g_e.md))
* **CreateScope** ([**AGE**](namespace_a_g_e.md))
* **count** ([**AGE**](namespace_a_g_e.md))


## d

* **DrawProperties** ([**AGE**](namespace_a_g_e.md))
* **DrawVec2Control** ([**AGE**](namespace_a_g_e.md))
* **DrawVec3Control** ([**AGE**](namespace_a_g_e.md))
* **DrawVec4Control** ([**AGE**](namespace_a_g_e.md))
* **dynamic\_unique\_cast** ([**AGE**](namespace_a_g_e.md))


## e

* **Error** ([**AGE::CoreLogger**](namespace_a_g_e_1_1_core_logger.md), [**AGE::GameLogger**](namespace_a_g_e_1_1_game_logger.md))
* **EWidgetStack** ([**AGE**](namespace_a_g_e.md))
* **EventCategory** ([**AGE**](namespace_a_g_e.md))
* **EventType** ([**AGE**](namespace_a_g_e.md))


## f

* **FloatingPointMicroseconds** ([**AGE**](namespace_a_g_e.md))
* **FramebufferTextureFormat** ([**AGE**](namespace_a_g_e.md))
* **FunctionToExecute** ([**AGE**](namespace_a_g_e.md))
* **for** ([**AGE**](namespace_a_g_e.md))


## g

* **GamePad** ([**AGE::GamePad**](namespace_a_g_e_1_1_game_pad.md))
* **GamePadCode** ([**AGE**](namespace_a_g_e.md))
* **GetFileFormat** ([**AGE**](namespace_a_g_e.md))
* **GetFormat** ([**AGE**](namespace_a_g_e.md))
* **g\_EditorAssetPath** ([**AGE**](namespace_a_g_e.md))


## i

* **Info** ([**AGE::CoreLogger**](namespace_a_g_e_1_1_core_logger.md), [**AGE::GameLogger**](namespace_a_g_e_1_1_game_logger.md))
* **IsDepthFormat** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **ImageFormat** ([**AGE**](namespace_a_g_e.md))
* **id** ([**AGE**](namespace_a_g_e.md))
* **INIT** ([**AK::BANKS**](namespace_a_k_1_1_b_a_n_k_s.md))


## j

* **JoyStickCode** ([**AGE**](namespace_a_g_e.md))
* **JoyStickID** ([**AGE**](namespace_a_g_e.md))


## k

* **Keys** ([**AGE::Key**](namespace_a_g_e_1_1_key.md))
* **KeyCode** ([**AGE**](namespace_a_g_e.md))


## l

* **LogType** ([**AGE**](namespace_a_g_e.md))


## m

* **MetaDataType** ([**AGE**](namespace_a_g_e.md))
* **MouseCode** ([**AGE**](namespace_a_g_e.md))
* **MM** ([**AK::BANKS**](namespace_a_k_1_1_b_a_n_k_s.md))
* **MASTER\_AUDIO\_BUS** ([**AK::BUSSES**](namespace_a_k_1_1_b_u_s_s_e_s.md))
* **MAIN\_MENU** ([**AK::EVENTS**](namespace_a_k_1_1_e_v_e_n_t_s.md))


## n

* **NodeArgType** ([**AGE**](namespace_a_g_e.md))
* **NO\_OUTPUT** ([**AK::AUDIO\_DEVICES**](namespace_a_k_1_1_a_u_d_i_o___d_e_v_i_c_e_s.md))


## o

* **OnRender** ([**AGE**](namespace_a_g_e.md))
* **operator\*** ([**AGE**](namespace_a_g_e.md))
* **operator+** ([**AGE**](namespace_a_g_e.md))
* **operator-** ([**AGE**](namespace_a_g_e.md))
* **operator&lt;&lt;** ([**AGE**](namespace_a_g_e.md))
* **operator^** ([**AGE**](namespace_a_g_e.md))


## p

* **PixelType** ([**AGE**](namespace_a_g_e.md))
* **PrintAudioDeviceInfo** ([**AGE**](namespace_a_g_e.md))
* **ProjectionType** ([**AGE**](namespace_a_g_e.md))
* **Proxy** ([**AGE**](namespace_a_g_e.md))
* **p** ([**AGE**](namespace_a_g_e.md))


## q

* **QC** ([**AGE**](namespace_a_g_e.md))
* **QueryBoxOverlap** ([**AGE**](namespace_a_g_e.md))
* **q** ([**AGE**](namespace_a_g_e.md))


## r

* **RTTR\_REGISTRATION** ([**AGE**](namespace_a_g_e.md))
* **Ref** ([**AGE**](namespace_a_g_e.md))
* **Rigid2DTypeToBox2DBody** ([**AGE**](namespace_a_g_e.md))
* **RigidBody2DBodyTypeToString** ([**AGE**](namespace_a_g_e.md))
* **RigidBody2DTypeFromString** ([**AGE**](namespace_a_g_e.md))


## s

* **State** ([**AGE::KeyState**](namespace_a_g_e_1_1_key_state.md))
* **SaveJsonFile&lt; GameFramework::EnemyStats &gt;** ([**AGE**](namespace_a_g_e.md))
* **SaveJsonFile&lt; GameFramework::ItemInfo &gt;** ([**AGE**](namespace_a_g_e.md))
* **SaveJsonFile&lt; GameFramework::PlayerStats &gt;** ([**AGE**](namespace_a_g_e.md))
* **SaveJsonFile&lt; Ref&lt; GameFramework::QuestInfo &gt; &gt;** ([**AGE**](namespace_a_g_e.md))
* **SafeRelease** ([**AGE**](namespace_a_g_e.md))
* **Scene\_Flag** ([**AGE**](namespace_a_g_e.md))
* **Scope** ([**AGE**](namespace_a_g_e.md))
* **SerializeEntity** ([**AGE**](namespace_a_g_e.md))
* **ShaderDataType** ([**AGE**](namespace_a_g_e.md))
* **ShaderDataTypeSize** ([**AGE**](namespace_a_g_e.md))
* **ShaderDataTypeToOpenGLBaseType** ([**AGE**](namespace_a_g_e.md))
* **ShaderTypeFromString** ([**AGE**](namespace_a_g_e.md))
* **s\_AudioScratchBuffer** ([**AGE**](namespace_a_g_e.md))
* **s\_AudioScratchBufferSize** ([**AGE**](namespace_a_g_e.md))
* **s\_Device** ([**AGE**](namespace_a_g_e.md))
* **s\_Engine** ([**AGE**](namespace_a_g_e.md))
* **s\_MaxFramebufferSize** ([**AGE**](namespace_a_g_e.md))
* **s\_Mp3d** ([**AGE**](namespace_a_g_e.md))
* **s\_RandomDevice** ([**AGE**](namespace_a_g_e.md))
* **s\_UniformDistribution** ([**AGE**](namespace_a_g_e.md))
* **SYSTEM** ([**AK::AUDIO\_DEVICES**](namespace_a_k_1_1_a_u_d_i_o___d_e_v_i_c_e_s.md))
* **SS\_AIR\_FEAR** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_FREEFALL** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_FURY** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_MONTH** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_PRESENCE** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_RPM** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_SIZE** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_STORM** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_TIMEOFDAY** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))
* **SS\_AIR\_TURBULENCE** ([**AK::GAME\_PARAMETERS**](namespace_a_k_1_1_g_a_m_e___p_a_r_a_m_e_t_e_r_s.md))


## t

* **Type** ([**AGE::Binding**](namespace_a_g_e_1_1_binding.md))
* **Trace** ([**AGE::CoreLogger**](namespace_a_g_e_1_1_core_logger.md), [**AGE::GameLogger**](namespace_a_g_e_1_1_game_logger.md))
* **TextureTarget** ([**AGE::Utils**](namespace_a_g_e_1_1_utils.md))
* **TargetPlatform** ([**AGE**](namespace_a_g_e.md))
* **Trans** ([**AGE**](namespace_a_g_e.md))


## u

* **ulong\_t** ([**AGE**](namespace_a_g_e.md))


## v

* **VariantBase** ([**AGE**](namespace_a_g_e.md))
* **VertexPositionColor** ([**AGE**](namespace_a_g_e.md))
* **VertexPositionColorTangent** ([**AGE**](namespace_a_g_e.md))
* **void** ([**AGE**](namespace_a_g_e.md))


## w

* **Warn** ([**AGE::CoreLogger**](namespace_a_g_e_1_1_core_logger.md), [**AGE::GameLogger**](namespace_a_g_e_1_1_game_logger.md))


## x

* **XInputMasks** ([**AGE::GamePad**](namespace_a_g_e_1_1_game_pad.md))


## ~

* **~AGEWindow** ([**AGE**](namespace_a_g_e.md))


## _

* **\_\_pad0\_\_** ([**AGE**](namespace_a_g_e.md))
* **\_\_pad1\_\_** ([**AGE**](namespace_a_g_e.md))
* **\_\_pad2\_\_** ([**AGE**](namespace_a_g_e.md))




